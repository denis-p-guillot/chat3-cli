...
export function buildPurpleCloudProposalRequest(form: ProposalFormState): string {
  ...
  const diagramSpec =
    'Include **at least one** fenced **PlantUML** diagram (` ```plantuml ` … ` ``` `) in this section: **(a)** reference topology — **Internet / users** → **Cloudflare** (edge) → **Odoo** (workers as sized) → **PostgreSQL** + **filestore**; **(b)** show **Git** (GitHub/GitLab) as the deploy source into the PurpleCloud pipeline when relevant; **(c)** use **package** / **rectangle** groupings only for environments that are **in scope** (Development / Staging / Production), each labeled **VALUE** or **PERFORMANCE** per the confirmed inputs—omit absent environments; **Odoo worker counts shown in the diagram must match the sizing tables above exactly** (no other numbers). Keep the graph readable (avoid more than ~12 nodes). Use a **left-to-right** or **top-down** deployment diagram (`left to right direction` or `top to bottom direction`). Wrap the diagram in `@startuml` / `@enduml`. After the diagram, add a **one-sentence caption** in the proposal language. Optionally add a **second small** PlantUML (e.g. promotion **Dev → Staging → Production** with only included stages).'
  ...
  return [
    ...
    '',
    'You are drafting a commercial proposal for a **dedicated Odoo** hosting deployment using **PurpleCloud** (https://purple-cloud.ai): an Odoo-focused cloud platform with dedicated servers, automated backups, security (including Cloudflare protection), monitoring, Git-based CI/CD, and separate environments (development, staging, production). **Always** illustrate the proposed infrastructure with **PlantUML** diagram(s) in the Architecture section (see deliverable item 3). Where relevant, surface **integrated console services** (monitoring, PostgreSQL insight, backup/restore, Web SSH) using the **Wiki reference block** below—**with Markdown links**—so due-diligence readers can verify claims.',
    '',
    ...
    ...(form.includeStagingInstance || form.includeDevInstance
      ? [
          '',
          `- **Staging & development specs/pricing:** when included, use the **Staging & development (catalog-backed rows)** block after the primary grid—each environment is matched to a **real catalog row** in the same tier (proxy ÷${5} / ÷${8} on **Light users** for selection only); copy **Odoo workers** and **USD** from that block only.`,
        ]
      : []),
    '',
    gridSection,
